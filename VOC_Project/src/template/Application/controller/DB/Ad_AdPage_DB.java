...
public class Ad_AdPage_DB {
	...
	public static boolean addNewMovie(String title, String genre, String director, int agegroup, String story,
		...
		connect.beginConnection();
		if (connect.conn != null) {
			String sql = "INSERT INTO movies (movies_id, title, genre, director, age_group, story, average_score, gee, open_date, production, image_file_name, schedule_date, schedule_time, running_time) "
					+ "values " + "(MOVIE_SEQ.nextval," + "'" + title + "', '" + genre + "', '" + director + "', "
					+ agegroup + ", '" + story + "', 0,'" + gee + "', '" + opendate + "', '" + production + "', '"
					+ imagefilename + "', '2021/06/15', 1, '" + time + "')";
